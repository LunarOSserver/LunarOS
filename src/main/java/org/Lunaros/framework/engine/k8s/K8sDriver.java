...
import org.Lunaros.framework.api.model.deployment.related.DeployEventStatus;
import org.Lunaros.framework.api.model.deployment.related.DeployOperation;
import org.Lunaros.framework.api.model.deployment.related.DeploymentSnapshot;
import org.Lunaros.framework.api.model.deployment.related.DeploymentStatus;
import org.Lunaros.framework.api.model.deployment.related.NetworkMode;
...
import org.Lunaros.framework.engine.k8s.util.SecretUtils;
...
public class K8sDriver implements RuntimeDriver {
    ...
    public void startDeploy(Deployment deployment, Version version, User user, List<EnvDraft> allExtraEnvs)
        ...
        try {
            ...
            // judge the registry is belong to Lunaros or not
...
//        }
    ...
    private void checkSecret(KubeUtils client, Version version, Deployment deployment) throws DeploymentEventException {
        if (version != null && SecretUtils.haveLunarOSRegistry(version.getContainerDrafts())) {
            try {
                if (client.secretInfo(GlobalConstant.SECRET_NAME_PREFIX + deployment.getNamespace()) == null) {
                    Map<String, String> dataMap = new HashMap<>();
                    dataMap.put(GlobalConstant.SECRET_DOCKERCFG_DATA_KEY, SecretUtils.getLunarOSImageSecretData());
