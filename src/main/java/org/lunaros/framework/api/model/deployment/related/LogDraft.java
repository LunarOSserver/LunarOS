...
public class LogDraft {
    ...
    public static List<EnvVar> formatContainerLogEnv(String kafkaBrokers, List<ContainerDraft> containerDrafts) {
            ...
            for (LogItemDraft logItemDraft : containerDraft.getLogItemDrafts()) {
                ...
                if (logItemDraft.isAutoCollect()) {
                    ...
                    EnvVar tmpEnv = new EnvVarBuilder()
                            .withName(String.format("LunarOS_FLUME_LOGFILE%d", idxFlumeSuffix))
                            .withValue(logFileName)
                            .build();
                    envs.add(tmpEnv);
                    tmpEnv = new EnvVarBuilder()
                            .withName(String.format("LunarOS_FLUME_TOPIC%d", idxFlumeSuffix))
                            .withValue(logItemDraft.getLogTopic())
                            .build();
                    envs.add(tmpEnv);
                    if (!StringUtils.isBlank(logItemDraft.getProcessCmd())) {
                        tmpEnv = new EnvVarBuilder()
                                .withName(String.format("LunarOS_FLUME_MORECMD%d", idxFlumeSuffix))
                ...
                if (logItemDraft.isAutoDelete()) {
                    String logFileName = FLUME_MOUNT_PATH_PREFIX + idxSuffix
                            + "/" + LogItemDraft.getLogFileName(logItemDraft.getLogPath());
                    EnvVar tmpEnv = new EnvVarBuilder()
                            .withName(String.format("LunarOS_CLEAN_LOGFILE%d", idxCleanSuffix))
                            .withValue(logFileName)
                            .build();
                    envs.add(tmpEnv);
                    tmpEnv = new EnvVarBuilder()
                            .withName(String.format("LunarOS_CLEAN_EXPIRETIME%d", idxCleanSuffix))
        ...
        if (idxFlumeSuffix > 1) {
            EnvVar tmpEnv = new EnvVarBuilder()
                    .withName("LunarOS_FLUME_LOG_COUNT")
                    .withValue(Integer.toString(idxFlumeSuffix-1))
                    .build();
            envs.add(tmpEnv);
            // kafka
            tmpEnv = new EnvVarBuilder()
                    .withName("LunarOS_FLUME_BROKER")
        ...
        if (idxCleanSuffix > 1) {
            // delete log count
            EnvVar tmpEnv = new EnvVarBuilder()
                    .withName("LunarOS_CLEAN_LOG_COUNT")
...
//        tmpEnv = new EnvVar();
//        tmpEnv.putName("LunarOS_FLUME_CHANNEL_DIR").putValue("/log");
//        envs.add(tmpEnv);
        return envs;
    }
