...
import org.Lunaros.exception.GitlabTokenException;
import org.Lunaros.exception.ProjectHookException;
...
import org.Lunaros.util.HttpsClient;
...
public class GitlabApiWrapper implements CodeApiInterface {
    ...
    public List<CodeSourceInfo> listCodeInfo(int userId) {
        List<org.Lunaros.framework.api.model.project.GitlabUser> gitlabs = GitlabInfo.getGitlabsByUserId(userId);
        return listCodeInfo(gitlabs);
    }

    @Override
    public List<CodeSourceInfo> listCodeInfo(int userId, int gitlabId) {
        List<org.Lunaros.framework.api.model.project.GitlabUser> gitlabs =
                GitlabInfo.getGitlabsByUserIdAndGitlabId(userId, gitlabId);
        return listCodeInfo(gitlabs);
    }

    private List<CodeSourceInfo> listCodeInfo(List<org.Lunaros.framework.api.model.project.GitlabUser> gitlabs) {
        if (gitlabs == null || gitlabs.isEmpty()) {
            return new ArrayList<>(1);
        }
        List<CodeSourceTask> codeSourceTasks = new ArrayList<>(gitlabs.size());
        for (org.Lunaros.framework.api.model.project.GitlabUser gitlab : gitlabs) {
    ...
    public void deleteDeployKeys(int projectId) {
        try {
            ...
            if (deployKeys != null) {
                for (GitlabSSHKey sshKey : deployKeys) {
                    if ("LunarOS".equals(sshKey.getTitle())) {
