window.serverSupportsSSL = false;
window.disableSSLMenu = false;
window.suppressAllAudio = false;
window.linkifyURLs = 1;
window.userCSSList = [];
window.serverMessagesOrigin = false;
...
VT100.prototype.initializeElements = function(container) {
  ...
  var widthDiff = ((window.innerWidth ||
    ...
    document.body.clientWidth) - marginRight) - (x + this.container.offsetWidth);
...
VT100.prototype.about = function() {
  alert("VT100 Terminal Emulator " + "2.19 " +
    ...
    if (part1 != undefined) {
      ch = part1 +
        ((event.shiftKey ? 1 : 0) +
          ...
          (event.ctrlKey ? 4 : 0)) +
        part2;
    } else if (ch.length == 1 && (event.altKey || event.metaKey) && !this.disableAlt) {
      ch = '\u001B' + ch;
    }
  }
